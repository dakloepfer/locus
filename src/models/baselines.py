...
class PCA_Baseline(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pca_path = (
            f"{config.MODEL.TYPE}_{config.MODEL.FEATURE_DIM}features_pca.pkl"
        )
    ...
    def make_pca(self, imgs):  # TODO also load samples here
        if os.path.exists(self.pca_path):
            ...
            with torch.no_grad():
                n_batches = 0
                for batch in tqdm(
                    loader, f"Getting the PCA for the {self.config.MODEL.TYPE} baseline"
                ):
                    imgs = (
                        batch["img"].to("cuda:0")
...
class DINO_Baseline(PCA_Baseline):
    ...
    def __init__(self, config):
        ...
        n_blocks = config.MODEL.N_PRETRAINED_BLOCKS
        model = config.MODEL.VIT_BACKBONE
